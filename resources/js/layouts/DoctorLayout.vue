...
<template>
    <div class="min-h-screen bg-gray-50 dark:bg-gray-950">
        <!-- Sidebar -->
        <aside class="fixed inset-y-0 left-0 z-50 w-64 border-r border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
            <div class="flex h-full flex-col">
                ...
                <div class="border-t border-gray-100 p-4 dark:border-gray-800">
                    <div class="flex items-center gap-3 rounded-xl p-3">
                        <div class="flex h-8 w-8 items-center justify-center rounded-full bg-violet-100 text-sm font-semibold text-violet-700 dark:bg-violet-900/40 dark:text-violet-300">
                            {{ user?.name?.charAt(0)?.toUpperCase() ?? 'D' }}
                        </div>
                        <div class="min-w-0 flex-1">
                            <p class="truncate text-sm font-medium text-gray-900 dark:text-gray-100">{{ user?.name }}</p>
                            <p class="truncate text-xs text-gray-500 dark:text-gray-500">{{ user?.email }}</p>
                        </div>
                        <Link href="/logout" method="post" as="button" class="text-gray-400 transition-colors hover:text-gray-600 dark:text-gray-600 dark:hover:text-gray-400">
                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="pl-64">
            <!-- Top bar -->
            <header class="sticky top-0 z-40 flex h-16 items-center justify-between border-b border-gray-200 bg-white px-6 dark:border-gray-800 dark:bg-gray-900">
                <slot name="header">
